...
// Server-Side Intelligence Scoring (8-Layer Algorithm)
...
app.get('/api/news', async (req, res) => {
    try {
        ...
        const articles = await Article.find(query).sort({ publishedAt: -1 }).limit(2000);
        res.json(articles);
    } catch (err) {
...
app.get('/api/sitrep', async (req, res) => {
    try {
        // SITREP considers the last 7 days of global intel
        const timeLimit = Date.now() - (7 * 24 * 60 * 60 * 1000);
        const articles = await Article.find({ publishedAt: { $gte: timeLimit } });
        
        // Execute server-side map-reduce scoring
        const scoredArticles = articles.map(a => {
            const doc = a.toObject();
            doc.relevanceScore = calculateRelevance(doc);
            return doc;
        });
        
        // Sort and deliver exactly 15 highest-value items
        scoredArticles.sort((a, b) => b.relevanceScore - a.relevanceScore);
        res.json(scoredArticles.slice(0, 15));
    } catch (err) {
